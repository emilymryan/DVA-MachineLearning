...
#         x = x[step:]
    ...
    plt.ylim(0,0.5)
#     plt.show()
    plt.savefig('/projectnb/ryanlab/jackz/test/Diff_V/BO_vs_GD/NMC532-Gr_GD/combined_plots_%s/DVQ %s cyc %s acq %s idx %s.png'%(str(fs),cd,str(cycnum),str(acq),str(idx)))
    plt.figure().clear()
...
#             print(gain_C)
    ...
    #             rb_list = []
        ...
        def loss_wrapper(params):
            return loss(*params)
        #init by charging
#         random_values = {'kc': 0.9368127764123356,
#  'bc': 0.18075778599217193,
#  'r': 0.013180754270739926}
        #init by discharging
        #TODO: change this to true random 
        random_values = {'kc': 0.8558367667952452,'bc': -0.716801253052069,'ka': 0.9558367667952452,'ba': -0.716801253052069,'r': 0.0353010311050839}
        # Generate a random value within the range for each parameter
#         random_values = {key: random.uniform(*value) for key, value in pbounds.items()}
        
        initial_guess = []
        param_order = ['ka','ba','kc','bc','r']
        for param in param_order:
            initial_guess.append(random_values[param])


#             result = minimize(loss_wrapper, initial_guess, method='Nelder-Mead')
        result = minimize(loss_wrapper, initial_guess, method='L-BFGS-B', bounds=[pbounds[key] for key in param_order])
#             print(result)
#             print('======================')
...
#             rb=optimizer.max['params']['rb']
    ...
    #             plt.ylim(-0.4,0)
    #             plt.show()
        plt.savefig('/projectnb/ryanlab/jackz/test/Diff_V/BO_vs_GD/NMC532-Gr_GD/combined_plots_%s/VQ Discharge cyc %s acq %s idx %s.png'%(str(fs),str(cycnum),str(acq),str(idx)))
        plt.figure().clear()
        ...
        print(ttl_lst[count])
        plt.title(' %s, kc=%s, bc=%s, ka=%s, ba = %s' %(cycnum,str(round(kc,rn)),str(round(bc,rn)), str(round(ka,rn)),str(round(ba,rn))))
        plt.savefig('/projectnb/ryanlab/jackz/test/Diff_V/BO_vs_GD/NMC532-Gr_GD/combined_plots_%s/VQ Charge cyc %s acq %s idx %s.png'%(str(fs),str(cycnum),str(acq),str(idx)))
    #             annotations = [
    #                 "length x = " +  str(len(dVdQ_cyc)),
    #                 "min x = " + str(min(x)),
    #                 "max x = " + str(max(x))
...
#         plt.figure().clear()
    ...
    f = open("/projectnb/ryanlab/jackz/test/Diff_V/BO_vs_GD/NMC532-Gr_GD/combined_dicts_%s/combined %s %s %s resistance_dict.pkl" %(str(fs), str(cycnum),str(acq),str(idx)),"wb")
